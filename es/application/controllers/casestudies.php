...
function __construct()
	...
	public function index()
	{
		...
		$data['description'] = 'Wheelwash wheel cleaning solutions - A few case studies showcasing some of our work. ' . $data['casestudies'][0]->title . '. ' . $data['casestudies'][1]->title . '. ' . $data['casestudies'][2]->title . '. ' . $data['casestudies'][3]->title;
	...
	public function show($id)
	{
		...
		$data['title'] = 'Case Studies';
		$data['description'] = 'Wheelwash wheel cleaning solutions - A few case studies showcasing some of our work. ' . $data['case']['title'] . ' - ' . $data['case']['content'];
		$this->load->view('header', $data);
	...
	public function getone($id)
	{
		...
		echo '<h3>' . $casestudies['title'] . '<span>' . substr($casestudies['modified_date'], 0, 10) . '</span></h3><div class="summary">' . substr($casestudies['content'], 0, 255) . '</div><div class="content" style="display:none;">' . $casestudies['content'] . '</div><div class="image" style="display:none;"><img src="' . $this->config->item('outside_base_url') . 'uploads/' . $casestudies['image_link'] . '" /></div><div class="circle" style="display:none;"><img src="' . $this->config->item('outside_base_url') . 'uploads/' . $casestudies['circle_image_1'] . '" /> <img src="' . $this->config->item('outside_base_url') . 'uploads/' . $casestudies['circle_image_2'] . '" /> <img src="' . $this->config->item('outside_base_url') . 'uploads/' . $casestudies['circle_image_3'] . '" /></div>';
	}
